...
void SmartConverter::copyAndCalculate() {
    ...
    double memoryLimitInPixels = memoryLimitInBytes / (2.0 * sizeof(float));
    ...
    for (unsigned int s = 0; s < stokes; s++) {
        ...
        for (hsize_t block = 0; block < n_blocks; block++ ) {
            ...
            for(hsize_t c = c_start; c < c_end; c++) {                 
                ...
                float* standardSlice = standardCube + block_pos;
                float* rotatedSlice = rotatedCube + block_pos;
                ...
                // rotate channel:
                // calculateRotatedChannel(s, c_start, c_end, standardCube, rotatedCube, sliceIncrement);
        ...
        // ADD THESE LINES: Write finalized Z-stats to disk
        if (depth > 1) {
...
double SmartConverter::calculateRotatedChannel( unsigned int s, hsize_t c_start, hsize_t c_end, float* standardCube, float* rotatedCube, int sliceIncrement )
{   
   hsize_t image_size = width*height;
// 1. Fast exit if no rotation is needed. 
    // This removes the `if (depth > 1)` check from the innermost loop, saving millions of branch evaluations.
    if (depth <= 1) {
        return -1.00; 
    }
    ...
    // 3. Pre-calculate strides outside the loops to avoid redundant multiplications
    const size_t src_z_stride  = width * height;
    const size_t src_y_stride  = width;
    
    const size_t dest_x_stride = height * sliceIncrement;
    const size_t dest_y_stride = sliceIncrement;

    // 4. Collapse the outer block loops so OpenMP has a rich pool of independent tasks
    #pragma omp parallel for collapse(3) schedule(dynamic)
    for (hsize_t i0 = 0; i0 < n_channels; i0 += BLOCK_SIZE) {
        for (hsize_t j0 = 0; j0 < height; j0 += BLOCK_SIZE) {
            for (hsize_t k0 = 0; k0 < width; k0 += BLOCK_SIZE) {
                
                // Calculate boundaries for the inner loops to prevent out-of-bounds segfaults
                // if dimensions are not perfect multiples of BLOCK_SIZE.
                hsize_t i_max = std::min(i0 + BLOCK_SIZE, n_channels);
                hsize_t j_max = std::min(j0 + BLOCK_SIZE, height);
                hsize_t k_max = std::min(k0 + BLOCK_SIZE, width);
                ...
                for (hsize_t i = i0; i < i_max; ++i) {
                    for (hsize_t j = j0; j < j_max; ++j) {
                        ...
                        // Precompute invariant base indices for this specific (i, j)
                        size_t dest_base_idx = i + dest_y_stride * j;
                        size_t src_base_idx  = i * src_z_stride + j * src_y_stride;

                        // Hint to the compiler to vectorize this innermost loop (AVX/SSE)
                        #pragma omp simd
                        for (hsize_t k = k0; k < k_max; ++k) {
                            
                            size_t destIndex = dest_base_idx + dest_x_stride * k;
                            size_t srcIndex  = src_base_idx + k;
                            
                            rotatedCube[destIndex] = standardCube[srcIndex];
                        }
                    }
                }
            }
